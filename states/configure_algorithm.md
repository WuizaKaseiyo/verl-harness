...
3. **Surface knobs in HITL.** Display the skill's "Knobs to surface in `configure_algorithm`" table with the recipe's current default values. The user may override any value. Algorithm-specific knobs add to (not replace) the `## Key hyperparameters` block in `recipe.md` — the union is what `launch_training` will splice into the CLI. **Refinement re-entry:** when this state is entered from `reflect`, apply the approved delta in `workspace/reflect/refinement_plan.md` as user-provided overrides in this step (its knobs are a subset of this table by construction), and note the iteration number in the presentation.
