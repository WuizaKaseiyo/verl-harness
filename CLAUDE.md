...
This repo contains **no executable code**. It is a set of markdown specs that drive an agent through a [verl](https://github.com/volcengine/verl) training run end-to-end. Every `.md` under `states/` and `skills/` is a runtime instruction file the agent reads and executes — there is nothing to build, lint, or unit-test. There is no test runner, no package manager, no entry-point script. `runs/` is git-ignored scratch space for per-execution workspaces.
...
The harness is a finite state machine. The diagram lives in `task-overview.md`:
...
intake → locate_recipe → prepare_data ⇄ generate_preprocess
        ...
        select_compute → provision_env → launch_training → monitor_training → summarize → finalize
                              │                 │
                              └─ env_failed ────┴─ launch_failed ─→ finalize (short-circuit)
```
...
3. **Compute target** (`select_compute` + `compute_local`/`compute_slurm`/`compute_ssh_slurm`). Three targets — `local-direct`, `local-slurm`, `ssh-slurm` — gated by three probes (`nvidia-smi`, `sinfo`+`squeue`, `ssh <alias> sinfo`). `auto` ranking is local-direct > local-slurm > ssh-slurm. `provision_env`, `launch_training`, and `monitor_training` each pull in **only the one** compute skill matching the chosen target — the other two are listed in those states' `## Skills` blocks for validator coverage but are explicitly not consulted on the run.
